...
class LineScan:
    ...
    def __init__(self, axis_name, start, stop, step, hp_avgs=100, hp_range=0.1, mc=None):
        ...
        self.stop = stop
        self.step = step
        self.n_steps = int(abs((self.stop - self.start) / self.step + 1))
        self.pos_values = np.linspace(self.start, self.stop, self.n_steps)
        ...
        if axis_name in ('x', 'z'):
            ...
            speed = min(self.step / 0.1, self.axis.max_speed)  # set time longer if get MissedTriggerErrors
